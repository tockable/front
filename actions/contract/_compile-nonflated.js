...
async function createCostimizedContractFile(
  ...
  let editedBaseContract = baseContract.replace(
    /contract TockableDrop is ERC721AQueryable, Ownable, ReentrancyGuard {/g,
    `contract ${_contractName} is ERC721AQueryable, Ownable, ReentrancyGuard {`
  );

  editedBaseContract = baseContract.replace(
...
export async function _createAndCompile(_creator, _uuid) {
  try {
    let project;
